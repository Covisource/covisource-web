...
const SearchBar = () => {
  ...
  return (
    <>
      {/* Large Search Bar */}
      <div
        className="relative hidden md:flex items-center ct-shadow transition-all mb-10 bg-white rounded-xl sm:w-35 md:w-144 lg:w-160 h-18"
        id="searchbar_home"
      >
        <SearchablePopup
          input={{
      ...
      <div className="relative md:hidden w-full">
        <div className="flex flex-col gap-3 p-6 m-4 bg-white ct-shadow rounded-xl">
          <SearchablePopup
            input={{
              className: "w-full",
              subClassName:
          ...
          />
          <span
            className="w-full mx-3 ct-bg-muted"
            style={{ height: "1px" }}
          ></span>
          <SearchablePopup
            input={{
              subClassName:
                "ct-text-color-1 text-sm font-medium bg-transparent",
              prepend: (
                <i className="fas fa-shapes ct-text-color-1 text-lg"></i>
              ),
              heading: "Resource",
              placeholder: "Choose a Resource",
            }}
        ...
        <Button
          className="gap-2 ct-bg-accent absolute right-2 py-4 rounded-xl text-gray-200 ct-shadow"
          href={`${process.env.NEXT_PUBLIC_BASE_URL}/${
            selectedResource.toLowerCase() || ""
          }`}
        >
